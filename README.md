...
### 🎯 项目分析功能
强大的项目分析工具，帮助您监控项目进度和识别潜在问题。

#### 关键路径分析
- **自动识别关键路径** - 准确计算项目的关键路径和关键任务
- **浮动时间计算** - 计算每个任务的总浮动时间和自由浮动时间
- **项目工期预测** - 准确预测项目总工期和项目结束日期
- **详细时间分析** - 显示每个任务的最早开始、最早完成、最晚开始、最晚完成时间
- **可视化展示** - 关键路径以流程图形式清晰展示
- **数据导出** - 支持将分析结果导出为 CSV 文件

#### 依赖关系验证
- **循环依赖检测** - 自动检测并提示任务间的循环依赖问题
- **无效连接识别** - 识别缺失任务或无效的依赖关系
- **日期冲突检查** - 验证依赖关系与任务日期的一致性
- **依赖类型显示** - 清晰显示每个依赖关系的类型（FS/SS/FF/SF）
- **错误分级** - 区分严重错误和一般错误，优先处理关键问题
- **解决方案建议** - 为每个错误提供具体的解决方案
- **冗余依赖提示** - 识别可能冗余的依赖关系并给出优化建议
- **详细报告导出** - 支持导出完整的验证报告

#### 任务约束验证
- **多种约束类型** - 支持 SNET、SNLT、FNET、FNLT、MSO、MFO、ASAP、ALAP 等约束
- **约束冲突检测** - 自动检测任务是否违反时间约束
- **详细冲突信息** - 显示当前日期与约束日期的对比
- **筛选和排序** - 支持按冲突类型筛选和排序任务
- **约束类型说明** - 内置约束类型说明，帮助理解各种约束的含义
- **警告提示** - 对可能存在问题的约束给出警告和建议
- **报告导出** - 支持导出约束验证报告

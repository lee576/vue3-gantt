...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getChildIndex = (parentId: string, childId: string): number => {
      const siblings = store.tasks.filter(
        task =>
          task[store.mapFields.parentId] === parentId && task[store.mapFields.parentId] !== '0'
      )
    ...
    // 完成-开始连线路径（支持不同路径类型）
    const generateFinishToStartPath = (
      source: any,
      target: any,
      pathType: LinkPathType
    ): string => {
    ...
    // 完成-完成连线路径（支持不同路径类型）
    const generateFinishToFinishPath = (
      source: any,
      target: any,
      pathType: LinkPathType
    ): string => {
    ...
    // 开始-完成连线路径（支持不同路径类型）
    const generateStartToFinishPath = (
      source: any,
      target: any,
      pathType: LinkPathType
    ): string => {
    ...
    const generateConnectionPath = (
      ...
      switch (pathType) {
        ...
        case LinkPathType.BEZIER:
          return generateBezierConnectionPath(
            startX,
            startY,
            endX,
            endY,
        ...
        case LinkPathType.RIGHT_ANGLE:
        default:
          return generateRightAngleConnectionPath(
            startX,
            startY,
            endX,
            endY,
    ...
    const updateLinks = () => {
      ...
      const shouldShowLinkType = (linkType: LinkType | number): boolean => {
        ...
        if (typeof linkType === 'number') {
          switch (linkType) {
            case 0:
              return visibility.finishToStart // FINISH_TO_START
            case 1:
              return visibility.startToStart // START_TO_START
            case 2:
              return visibility.finishToFinish // FINISH_TO_FINISH
            case 3:
              return visibility.startToFinish // START_TO_FINISH
            default:
              return true
          }
        }
    ...
    // 监听甘特图日期范围变化
    watch(
      () => [store.startGanttDate, store.endGanttDate],
      () => {
        setTimeout(updateLinks, 100)
      }
    )
    ...
    // 监听连线类型显示配置变化
    watch(
      () => props.linkConfig.linkTypeVisibility,
      () => {
        setTimeout(updateLinks, 50)
      },
      { deep: true }

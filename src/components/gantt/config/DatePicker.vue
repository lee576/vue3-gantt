      ...
      <div class="e-calendar" v-show="showCalendar" ref="calendarRef" :style="calendarPosition">
        <div class="e-date-select">
          ...
          <div class="e-date-monthday">
            <transition name="fadeY">
              <div
                :key="showDate.year + '-' + showDate.month"
                class="e-date-monthday-select"
                :class="{ active: !showYear }"
                @click="openCalendarList"
              >
                <span
                  >{{ keepDoubleDigit(showDate.month) }}-{{ keepDoubleDigit(selectDate.day) }}</span
                >&nbsp;
        ...
        <div class="e-calendar-container" v-show="!showYear && !showMonth">
            ...
            <div class="e-calendar-toolbar-title">
              <transition :name="fadeXType">
                ...
                >
                  ...
                  >&nbsp;
                  <span style="cursor: pointer" @click="openMonthList">{{
                    showDate.monthStr
                  }}</span>
                </div>
              </transition>
          ...
          <div class="e-calendar-week">
            <span
              v-for="(weekName, index) in getWeekNames"
              :key="index"
              class="e-calendar-week-day"
              >{{ weekName }}</span
            >
          </div>
            ...
            <div :key="updateKey" class="e-calendar-monthday-wrapper">
              <div v-for="(row, index) in rows" :key="index" class="e-calendar-monthday-row">
                <span
                  ...
                  @click="selectDay(day)"
                  @mouseenter="handleDayMouseEnter(dayIndex, index)"
        ...
        <ul class="e-calendar-year" v-show="showYear" ref="yearListRef">
          <li
            ...
            @mouseenter="handleYearMouseEnter(index)"
            @mouseleave="handleYearMouseLeave()"
          >
            {{ item }}
          </li>
        </ul>
        <ul class="e-calendar-year" v-show="showMonth" ref="monthListRef">
          <li
            ...
            @mouseenter="handleMonthMouseEnter(index)"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const isMinLimitMonth = () => {
      return (
        displayYear.value < minYear.value ||
    ...
    const isMaxLimitMonth = () => {
      return (
        displayYear.value > maxYear.value ||
    ...
    const rows = computed(() => {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (year === minYear.value && month === minMonth.value && day < minDay.value)
          isDisabled = true
        if (year === maxYear.value && month === maxMonth.value && day > maxDay.value)
          isDisabled = true
    ...
    const updateSelectedDateText = () => {
      ...
      selectedDateText.value = DateUtils.format(dateStr, fmt)
    }
    ...
    const selectDay = (cell: DayCell) => {
      ...
      const rawWeek = new Date(
        selectedYear.value,
        selectedMonth.value - 1,
        selectedDay.value
      ).getDay()
      const week = rawWeek === 0 ? 7 : rawWeek
      const weekStr = DateUtils.format(`${selectedYear.value}-${selectedMonth.value}-${selectedDay.value}`, 'dddd')
    ...
    return {
      ...
      selectDate,
      showDate,
      copyMinDate: computed(() => ({
        year: minYear.value,
        month: minMonth.value,
        day: minDay.value,
      })),
      copyMaxDate: computed(() => ({
        year: maxYear.value,
        month: maxMonth.value,
        day: maxDay.value,
      })),
      fadeXType,
      nextMonthClick,
...
body
  > .e-calendar:not([data-gantt-theme='liquidGlass'])
  .e-calendar-monthday-row-day.pointer:hover {
